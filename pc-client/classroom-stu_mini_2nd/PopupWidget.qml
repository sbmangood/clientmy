...
Rectangle {
    ...
    TipEvaluateWidgetItem{
        ...
        visible: false
        z:5
        studentName: curriculumData.getUserName("0");
    ...
    TipWaitIntoClassRoom{
        ...
        anchors.fill: parent
        visible: true
        tagNameContent:waitWidgetContent == "0" ? "进入教室中…":(waitWidgetContent == "1" ? "正在同步教室信息......":"请求进入教室...")
        z:5
        onSigCloseAllWidget: {
            popupWidget.sigCloseAllWidget();
        }
    ...
    CurriculumData{
        ...
        onSigListAllUserId: {
            for(var j = 0; j < list.length ; j++) {
                ...
                videoToolBackground.updateIsVideo(uid,userOnline);
            }
        }
    }
