...
bool YMTCPSocket::initMarsService()
{
    ...
    if(TemporaryParameter::gestance()->m_ipList.count()>0){
      for(int i = 0; i < TemporaryParameter::gestance()->m_ipList.count(); ++i){
          allip.push_back(TemporaryParameter::gestance()->m_ipList.at(i).toString().toStdString());
      }
    }
