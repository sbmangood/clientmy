    ...
    , m_sysnStatus(false)
{
    ...
    m_enterRoomMsg = QString("1#SYSTEM{\"command\":\"enterRoom\",\"content\":"
                             ...
                             "\"deviceInfo\":\"%8\",\"lat\":\"%9\",\"lng\":\"%10\",\"netType\":\"%11\",\"appSource\":\"YIMI\"},\"domain\":\"system\"}").arg(StudentData::gestance()->m_appVersion).arg(StudentData::gestance()->m_mD5Pwd).arg(StudentData::gestance()-> m_selfStudent.m_studentId).arg("STU").arg(StudentData::gestance()->m_sysInfo)
        ...
        if(documet.isObject())
        {
            ...
            else if (domain == "control")
            {
                ...
                else if(command == "zoomInOut")
                {
                    ...
                    //重设 该页的滚动条位置
                    m_pages[m_currentCourse][m_currentPage].offsetY = offsetX;
...
QString SocketHandler::syncUserHistroyFinMsgTemplate()
{
 ...
 QString SocketHandler::avReqMsgTemplate(int playStatus, int timeSec, qint64 coursewareId,QString path,QString suffix)
  {
      quint64 lid = StudentData::gestance()->m_liveRoomId.toLongLong();
      quint64 uid = StudentData::gestance()->m_currentUserId.toLongLong();
...
QString SocketHandler::responderMsgTemplate(int respType, int timesec, QString userName)
{
    quint64 lid = StudentData::gestance()->m_liveRoomId.toLongLong();
    quint64 uid = StudentData::gestance()->m_currentUserId.toLongLong();
...
void SocketHandler::cacheDocInfo(QJsonArray coursewareUrls, QString coursewareId)
{
    if (m_pages.contains("DEFAULT"))
    {
        ...
        {
            m_pages[m_currentCourse].append(MessageModel(1, coursewareUrls.at(i).toString(), 1.0, 1.0,coursewareId, currentCourwareType,0,false));
        }
    }
    else if (!m_pages.contains(coursewareId))
    {
        QList<MessageModel> list;
        list.append(MessageModel(0, "", 1.0, 1.0, coursewareId,currentCourwareType,0,false));
    ...
    if(m_currentDocType == 3 && m_sysnStatus)
    {
...
void SocketHandler::cacheDocMessage(QString& fromUid, QString& message)
{
    ...
    {
        h5DocUrl = contentJsonObj.take(kSocketH5Url).toString();
    }
    m_currentDocType = docType;
    m_currentCourseUrl = h5DocUrl;
    emit sigH5Courseware(docType,h5DocUrl);
...
//    {
        ...
        if((upState != -1) && (trailState != -1) && (audioState != -1) && (videoState != -1))
        {
            ...
            emit sigAuthChange(qsUid,upState,trailState,audioState,videoState,m_sysnStatus);
        }
//    }
}
...
void SocketHandler::cacheRoll(QString& fromUid, QString& message)
{
    ...
    QString userName = "";
    if(contentJsonObj.contains("name"))
    {
        userName = contentJsonObj.take("name").toString();
    }
    bool isSelecte = false;
    if(StudentData::gestance()->m_userName.contains(userName))
    {
        isSelecte = true;
    }
    YMQosApiMannager::gestance()->responderIsSuccess(fromUid,isSelecte,StudentData::gestance()->m_address);
    //缓存随机信息
}
...
void SocketHandler::cacheStartClass()
{
    ...
    QString saveFilePath = bufferFilePath + "/stuClassBegin.dll";
...
void SocketHandler::cacheZoomMessage(QString& fromUid, QString& message)
{
       ...
       contentJsonObj.contains(kSocketOffsetX) && contentJsonObj.contains(kSocketOffsetY))
    {
        ...
        m_pages[m_currentCourse][m_currentPage].offsetY = factorX;
...
bool SocketHandler::syncUserHistroyReq(QJsonArray& userHistroyData)
{
    ...
    qDebug() << "==syncUserHistroyReq==" << jsonObj;
...
void SocketHandler::parseMsg(QString& msg)
{
    ...
    if(error.error == QJsonParseError::NoError && documet.isObject())
    {
                ...
                if(syncUserHistroyReq(userHistroyMsgs))
                {
                    ...
                    for(int i = 0; i < size; ++i)
                    {
                        ...
                        if(kSocketTrail == hisCommand || kSocketPoint == hisCommand || kSocketDoc == hisCommand || kSocketAV == hisCommand ||
                                kSocketAuth == hisCommand || kSocketMuteAll == hisCommand || kSocketPage == hisCommand || kSocketPlayAnimation == hisCommand ||
        ...
        else if(kSocketAV == command || kSocketAuth == command || kSocketMuteAll == command || kSocketZoom == command || kSocketPlayAnimation == command ||
...
void SocketHandler::parseUserCommandOp(QString& command, QString& message)
{
    ...
    {
        cachePlayAnimation(fromUser,message);
    }
    else
    {
...
void SocketHandler::cachePlayAnimation(QString &fromUid, QString &message)
{
    QJsonParseError jsonParseError;
    QJsonDocument document = QJsonDocument::fromJson(message.toUtf8(), &jsonParseError);
    if(QJsonParseError::NoError != jsonParseError.error || !document.isObject())
    ...
    if(jsonObj.contains(kSocketContent))
    {
        ...
        m_currentStep = step;
        qDebug() << "==cachePlayAnimation==" << step << pageId;
...
void SocketHandler::updateUserState(QJsonValue content)
{
    ...
    if (contentArray.size() > 1)
    {
        foreach (QJsonValue value, contentArray)
        {
            ...
            {
                TemporaryParameter::gestance()->m_teacherIsOnline = (state == "1" ? true : false);
                emit sigTeaOnline(uid,state,TemporaryParameter::gestance()->m_teacherIsOnline);
            }
            //动态添加学生进入教室
    ...
    if (err.error == QJsonParseError::NoError)
    {
        ...
        if (domain == "draw" && command == "courware")
        {
            ...
            if (m_pages.contains("DEFAULT"))
            {
                ...
                {
#ifdef USE_OSS_AUTHENTICATION
                    m_pages[m_currentCourse].append(MessageModel(1, arr.at(i).toString(), 1.0, 1.0,"", currentCourwareType,0,false, 0));
#else
                    m_pages[m_currentCourse].append(MessageModel(1, arr.at(i).toString(), 1.0, 1.0,"", currentCourwareType,0,false));
#endif
                }
            }
...
#ifdef USE_OSS_AUTHENTICATION
                list.append(MessageModel(0, "", 1.0, 1.0,"", currentCourwareType, 0,false,0));
#else
                list.append(MessageModel(0, "", 1.0, 1.0,"", currentCourwareType,0,false));
#endif
               ...
               // qDebug() << "all lessonpaln column columnType" << columnType;
                ...
                if (m_pages.contains("DEFAULT"))
                {
                    ...
                    for (int i = 0; i < arr.size(); ++i)
                    {
                        //传入 新讲义需要的数据
#ifdef USE_OSS_AUTHENTICATION
                        MessageModel msg = MessageModel(1, "", 1.0, 1.0,"", currentCourwareType, 0,false,0);
#else
                        MessageModel msg = MessageModel(1, "", 1.0, 1.0,"", currentCourwareType,0,false);
#endif

                        if(columnType == 0)
                        {
                            msg.columnId = columnId.toInt();
                            msg.columnType = QString::number(columnType);
                            msg.pageIndex = i;
                            msg.questionId = "";
                            msg.resourceContents = arr.at(i).toObject();
                            //msg.setNewCourseware(columnId.toInt(), QString::number(columnType), i, "", arr.at(i).toObject());
                        }
                        else
                        {
                            msg.columnId = columnId.toInt();
                            msg.columnType = QString::number(columnType);
                            msg.pageIndex = i;
                            msg.questionId = arr.at(i).toString();
                            msg.resourceContents = arr.at(i).toObject();
                            //msg.setNewCourseware(columnId.toInt(),QString::number(columnType), i, arr.at(i).toString(), arr.at(i).toObject());
                        }
        ...
        {
#ifdef USE_OSS_AUTHENTICATION
            m_pages[m_currentCourse].insert(++m_currentPage, MessageModel(0, "", 1.0, 1.0,"","1", 0,false,0));
#else
            m_pages[m_currentCourse].insert(++m_currentPage, MessageModel(0, "", 1.0, 1.0,"","", 0,false));
#endif
        ...
        else if (domain == "page" && command == "delete")
        {
            ...
            if (m_pages[m_currentCourse].size() == 1)
            {
                ...
                //                model->release();
#ifdef USE_OSS_AUTHENTICATION
                m_pages[m_currentCourse].append(MessageModel(0, "", 1.0, 1.0,"","1", 0,false,0));
#else
                m_pages[m_currentCourse].append(MessageModel(0, "", 1.0, 1.0,"", "1",0,false));
#endif
                return;
            }
            m_pages[m_currentCourse][m_currentPage].release();
...
#ifdef USE_OSS_AUTHENTICATION
            m_pages[m_currentCourse].insert(++m_currentPage, MessageModel(0, url, width, height,"", "1", 0,false,0));
#else
            m_pages[m_currentCourse].insert(++m_currentPage, MessageModel(0, url, width, height,"", "1",0,false));
#endif
        }
...
void SocketHandler::justChangeIp(bool isSuccess)
{
    ...
    if(isSuccess)
    {
        YMQosApiMannager::gestance()->registerClassroomInfo(StudentData::gestance()->m_address);
        YMQosApiMannager::gestance()->enterClassroomSuccess(QString::number(StudentData::gestance()->m_startTime),QString::number(StudentData::gestance()->m_endTime),"1","进入教室成功",StudentData::gestance()->m_address);
    }
    ...
    if(isAutoChangeIping)
    {
        ...
        {
            YMQosApiMannager::gestance()->socketDisconnect("连接失败!",StudentData::gestance()->m_address);
            //autoChangeIpResult("autoChangeIpFail");
        }
...
{
    qDebug() << "====SocketHandler::restGoodIpList=====" <<TemporaryParameter::gestance()->goodIpList;
    //重设优化ip列表
    int tempHasConnect = -1;
    for(int a = 0; a < TemporaryParameter::gestance()->goodIpList.size(); a++ )
    {
