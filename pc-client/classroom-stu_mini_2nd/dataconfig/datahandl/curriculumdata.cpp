...
QString CurriculumData::justUserOnline(QString userId)
{
    ...
    qDebug() << "====userIdsidididid====" << userId << userIds << StudentData::gestance()->m_onlineId;
...
QJsonArray CurriculumData::getRosterInfo()
{
    ...
    for(int i = 0; i < StudentData::gestance()->m_allStudentUserInfo.size(); i++)
    {
        ...
        //如果是自己则不到花名册显示
//        if(userId == StudentData::gestance()->m_currentUserId)
//        {
//            continue;
//        }
...
QJsonArray CurriculumData::getAllStudentInfo()
{
    QJsonArray dataArray;
    QJsonObject contentData;
    QString userId;
...
{
    QString msgType = "XBKCloudClassroom_student_click_feedback";
    QJsonObject jsonObj;
