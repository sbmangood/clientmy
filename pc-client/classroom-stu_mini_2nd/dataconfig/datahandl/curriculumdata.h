...
class CurriculumData : public QObject
{
        ...
        //
        Q_PROPERTY(QString lessonType READ lessonType)
    public:
        ...
        //获取当前授权是不是 本地用户授权 true 为此次授权为本地用户授权  false 为其他用户授权
        Q_INVOKABLE bool getAuthType();

        //获取是否是主动断开服务器
        Q_INVOKABLE bool isAutoDisconnectServer();
        //获取当前用户ID
        Q_INVOKABLE QString getCurrentUserId();
        //获取uid
        Q_INVOKABLE int getUserUid(QString userId);
        //获取用户上下台
        Q_INVOKABLE QString getUserUpStatus(QString userId);
        //获取用户头像
        Q_INVOKABLE QString getUserPicture(QString userId);
        //获取花名册信息
        Q_INVOKABLE QJsonArray getRosterInfo();
        //获取奖杯数量
        Q_INVOKABLE QString getRewardNum(QString userId);

        //问题反馈上报
        Q_INVOKABLE void addFeedbackInfo(QString feedbackTest);

        //获取所有学生信息
        Q_INVOKABLE QJsonArray getAllStudentInfo();
