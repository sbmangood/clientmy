...
#else
MessageModel::MessageModel(int isCourware, QString bgimg, double width, double height,QString questionId, QString currentCoursewareType,double offsetY,bool questionBtnStatus) :
    isCourware(isCourware), bgimg(bgimg), width(width), height(height),questionId(questionId), currentCoursewareType(currentCoursewareType),offsetY(offsetY),questionBtnStatus(questionBtnStatus)
#endif
{
}
//向信息容器内添加数据
...
void MessageModel::setNewCourseware(int columnId, int columnType, int pageIndex, QString questionId, QJsonObject resourceContents)
{
    ...
    this->columnType = QString::number(columnType);
