...
signals:
    ...
    void sigAuthChange(QString userId,int up,int trail,int audio,int video,bool isSynComplete);//授权信号
    ...
    void sigH5Courseware(int coursewareType,QString h5Url);
    void sigTeaOnline(QString userId,QString isOnline,bool isStartLesson);//老师是否在线信号
...
signals:
    ...
    //设置播放动画
    void sigPlayAnimation(QString pageId,int step);
...
public:
     ...
     QString avReqMsgTemplate(int playStatus, int timeSec, qint64 coursewareId,QString path,QString suffix);
     ...
     QString responderMsgTemplate(int respType, int timesec, QString userName);
 ...
 private:
     ...
     void cachePlayAnimation(QString &fromUid,QString& message);
     //协议中的数据缓存管理-end
