...
//    QMessageBox::critical(NULL, "aaa", strAppFileName, QMessageBox::Ok, QMessageBox::Ok); //测试中文乱码
    ...
    QSharedMemory shared("attendclassroom"); //exe被复制粘贴以后, 还是只能运行一个
    if (shared.attach())
    {
        qDebug() << QString("attendclassroom has already been running, please exit and run again.") << __FILE__ << __LINE__;
        return 0;
    }
    shared.create(1);
    ...
    if (!QFile::exists(systemPublicFilePath + "stutempattend.ini"))
    {
        qDebug() << QString("File: %1 does not existed") .arg(systemPublicFilePath + "stutempattend.ini") << __FILE__ << __LINE__;
        return 0;
    }

    QFile file(systemPublicFilePath + "stutempattend.ini");
    if(!file.open(QIODevice::ReadOnly))
    {
        qDebug() << QString("Open file: %1 failed.") .arg(systemPublicFilePath + "stutempattend.ini") << __FILE__ << __LINE__;
        return 0;
    }
    ...
    file.close();
    // QFile::remove(systemPublicFilePath +"stutempattend.ini");
    ...
    YMQosManager::gestance()->initQosManager(backData,"audit");//初始化qosManager "tea/stu/cc/audit,
    //接入辅助进程
    ...
    if(outputStr.contains(prcName)){
        ...
        yimiPrcSvrprocess->startDetached(runPath, QStringList());
