...
bool DebugLog::doUpgrade_LocalLogInfo_To_Server_Once()
{
    ...
    if(NULL != pClsUploadLogThread)
    {
        ...
        pClsUploadLogThread->setHttpUrl(StudentData::gestance()->apiUrl);
...
bool DebugLog::saveLogUrl(QString logUrl)
{
    ...
    maps.insert("appType", QStringLiteral("PC旁听"));
    ...
    QString url = "http://" + StudentData::gestance()->apiUrl + "/log/saveLogUrl";
    ...
    {
        qDebug() << "DebugLog::saveLogUrl::fail" << dataObj;
    }
...
QString DebugLog::uploadLog_To_OSS(QString logPath)
{
    ...
    QString key = QString("appLog/%1/${%2}-${%3}_%4").arg(times.toString("yyyy/MM/dd")) .arg(StudentData::gestance()->m_lessonId) .arg("attendclassroom").arg(fileName);
