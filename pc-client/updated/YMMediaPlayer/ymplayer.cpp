...
void YMPlayer::run()
{
    ...
    qDebug() << QStringLiteral("总时长：") << ps->pformat_ctx->duration / 1000000;
    emit totalTime(ps->pformat_ctx->duration / 1000000);
    ...
    emit playFinished();
    //qDebug()<<"ssssssssssssssssssssdasasadads";
...
void YMPlayer::onRenderCallBack(QImage img, double pts, void *data)
{
    YMPlayer *p = (YMPlayer*)data;
    emit p->sigOnRender(img);

    if( currentTime != int(pts) )
    {
        currentTime = int(pts);

        emit p->currentBePlayedTime(currentTime);
    }
    if(currentTime >= p->totalTimes)
    {
        SDL_Event event;
