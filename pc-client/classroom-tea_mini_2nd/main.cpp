...
#include "./videovoice/AudioVideoCenter.h"
...
int main(int argc, char *argv[])
{
    ...
    QSharedMemory shared("miniClassroom"); //exe被复制粘贴以后, 还是只能运行一个
    if (shared.attach())
    {
    ...
    if (!QFile::exists(systemPublicFilePath + "miniTemp.ini")) //"stutemp.ini")){
    {
        qDebug() << QString("!QFile.exists") << systemPublicFilePath + "miniTemp.ini" << __LINE__;
        return 0;
    }

    QFile file(systemPublicFilePath + "miniTemp.ini"); //"stutemp.ini");
