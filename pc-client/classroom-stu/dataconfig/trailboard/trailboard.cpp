...
//控制本地摄像头
void TrailBoard::setOperationVideoOrAudio(QString userId, QString videos, QString audios,int pingValue)
{
    ...
    QString sendStr = QString("0#{\"command\":\"settinginfo\",\"content\":{\"infos\":{\"camera\":\"%1\",\"networktype\":\"1\",\"microphone\":\"%2\",\"volume\":\"3\",\"ishideapp\":\"0\",\"delay\":\"%4\"},\"userId\":\"%3\"},\"domain\":\"control\"}")
            .arg(videos).arg(audios).arg(StudentData::gestance()->m_selfStudent.m_studentId).arg(pingValue);
...
void TrailBoard::sendOpenCorrect(QString planId, QString columnId, QString questionId, QString childQuestionId, bool isOpen)
{
    if(m_handler != NULL)
    {
        if(isOpen)
        {
            ...
            m_handler->sendLocalMessage(str, true, false);
            QJsonObject dataObj = QJsonDocument::fromJson(str.toLatin1()).object();
            reNewCloudModifyPageData(dataObj.value("content").toObject(), true);
        }
        else
        {
...
void TrailBoard::selectedMenuCommand(int pageIndex, int planId, int cloumnId)
{
    ...
    if(m_handler != NULL)
    {
        pageIndex = m_handler->getCurrentCoursewarePage( QString::number(planId), QString::number(cloumnId) );
...
void TrailBoard::creatRoomFail()
{
    ...
    if (TemporaryParameter::gestance()->m_supplier == "1") {
        supplierName = "agora";
    }
    else if (TemporaryParameter::gestance()->m_supplier == "2")
    {
    ...
    QString msg = QString(QStringLiteral("{\"command\":\"changeAudioResponse\",\"domain\":\"control\",\"content\":{\"supplier\":\"%1\",\"audioName\":\"%2\",\"videoType\":\"%3\",\"responseStatus\":\"0\",\"message\":\"\"}}")).arg(TemporaryParameter::gestance()->m_supplier).arg(supplierName).arg(TemporaryParameter::gestance()->m_videoType);
    m_handler->sendLocalMessage(msg, true, false);
    qDebug() << msg << "TrailBoard::creatRoomFail";
}

void TrailBoard::creatRoomSuccess()
{
    QString chanelName = "agora";
    if(TemporaryParameter::gestance()->m_supplier == "2")
    {
        chanelName = "tecent";
    }else if(TemporaryParameter::gestance()->m_supplier == "3")
    ...
    QString msg = QString(QStringLiteral("{\"command\":\"changeAudioResponse\",\"domain\":\"control\",\"content\":{\"supplier\":\"%1\",\"audioName\":\"%2\",\"videoType\":\"%3\",\"responseStatus\":\"1\",\"message\":\"\"}}")).arg(TemporaryParameter::gestance()->m_supplier).arg(chanelName).arg(TemporaryParameter::gestance()->m_videoType);
    if(m_handler != NULL)
    {
        m_handler->sendLocalMessage(msg, true, false);
    }
    qDebug() << msg << "TrailBoard::creatRoomSuccess";
}
