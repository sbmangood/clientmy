...
public:
        ...
        if(error.error == QJsonParseError::NoError)
        {
            if(documet.isObject())
            {
                ...
                if(jsonObjs.contains("data"))
                {
                    ...
                    //=========================
                    //获取applicationType
                    if(!jsonObj.contains("applicationType")) //applicationType字段, 不存在的话
                    {
                        m_applicationType = -1;
                    }
                    else
                    ...
                    //=========================
                    //获取 subjectId
                    if(!jsonObj.contains("subjectId")) //subjectId, 不存在的话
                    {
                        m_subjectId = -1;
                    }
                    else
                    ...
                    qDebug() << "setDocumentParsing" << m_lessonType << m_applicationType << m_subjectId << __LINE__;
    ...
    {
        QMap<QString, QString>::iterator userTypeInfoIterator =  m_userTypeInfo.begin();
        for(int i = 0; userTypeInfoIterator != m_userTypeInfo.end(); userTypeInfoIterator++, i++)
        {
            if(userTypeInfoIterator.key() == userId)
            {
                return userTypeInfoIterator.value();
            }
        }
        return "";
    }
...
public:
    ...
    QString qqVoiceVersion = "6.0.0";// 腾讯SDK版本
    bool isUsingTencentV2 = false;// 正在使用腾讯V2通道
