...
void ResetIpSettingFile::getGoodIplist(QString backData)
{
    ...
    QUrl url("http://" + StudentData::gestance()->apiUrl + "/app/netWork/getIPList?");
    ...
    loop.exec();
    QString data = reply->readAll();
    //data ="{\"message\": \"\",\"data\": [ {\"port\": \"5122\",\"udpPort\": \"5120\",\"ip\": \"118.178.89.74\"},{\"port\": \"5122\",\"udpPort\": \"5120\", \"ip\": \"123.59.155.57\"} ],\"result\": \"success\",\"code\": -1}";
    ...
    qDebug() << QStringLiteral("优选ip列表：") << StudentData::gestance()->apiUrl << data << urls << TemporaryParameter::gestance()->goodIpList;
}
