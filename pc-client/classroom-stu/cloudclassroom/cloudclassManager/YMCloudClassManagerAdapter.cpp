    ...
    : QObject(parent)
{
    ...
    if(StudentData::gestance()->lessonListIsEmptys == false)
    {
        for(int a = 0; a < 3; a++ )
        {            
            getLessonList();//YMUserBaseInformation::id);
            ++m_currentGetCourseTimes;
            if(StudentData::gestance()->lessonListIsEmptys == false)
            {
                break;
            }
        ...
        if(StudentData::gestance()->lessonListIsEmptys)
        {
            emit sigGetLessonListFail();
        }
    }
...
void YMCloudClassManagerAdapter::saveStudentAnswer(int useTime, QString studentAnswer, QJsonObject answerParm, int isFinished, QString imageAnswerUrl, QString childQId)
{
    ...
    QDateTime currentTime = QDateTime::currentDateTime();
    QVariantMap  reqParm;
    reqParm.insert("lessonId", StudentData::gestance()->m_lessonId.toLongLong());
...
{
    qlonglong lessonId = commentParm.value("lessonId").toString().toLongLong();
    QString prePlanId = commentParm.value("prePlanId").toString();
    qlonglong itemId = commentParm.value("itemId").toString().toLongLong();
...
//    qDebug() << "YMCloudClassManagerAdapter::getLessonList::dataObj  11"<<StudentData::gestance()->lessonListIsEmptys<<dataObj ;
    ...
    if(m_currentGetCourseTimes == 2 && StudentData::gestance()->lessonListIsEmptys)
    {
        //信息上报
        QJsonObject obj;
        obj.insert("result","0");
...
void YMCloudClassManagerAdapter::justGetCourseIsSuccess()
{
    if(StudentData::gestance()->lessonListIsEmptys)
    {
        emit sigGetLessonListFail();
