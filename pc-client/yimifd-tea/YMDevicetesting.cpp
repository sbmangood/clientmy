...
void YMDeviceTesting::startOrStopVideoTest(bool isStart)
{
    if(isStart)
    {
        ...
        if(videoDeviceCollect != NULL)
        {
            //这里两个摄像头的时候, 如何切换到另一个摄像头, 默认一直都是外部摄像头
            //qDebug() << "YMDeviceTesting::startOrStopVideoTest" << videoDeviceCollect->getCount();
    ...
    if(type == 2)
    {
        ...
        if(audioDeviceCollection != NULL)
        {
            if(audioDeviceCollection->getCount() > 0)
            {
                ...
                //qDebug() << "YMDeviceTesting::getUserDeviceList" << dataArray << __LINE__;
                return dataArray;
            }
        }
        else
...
{
    qDebug() << "YMDeviceTesting::saveDeviceInformation" << strDeviceName << __LINE__;
