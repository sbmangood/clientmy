...
Window {
    ...
    YMMassgeRemindManager{
        ...
        onRemindNewChange: {
            tipsView.currentData = remindNew;
        }
        onSigStageInfo: {
    ...
    Item{
        ...
        MouseArea{
            width: parent.width
            height:  Cfg.TB_HEIGHT * heightRate//40 * heightRate
            visible: mainView.visible

            //提示: 已生成录播视频的Message Tip
            YMMessageTipsView{
                id: tipsView
                visible: false
                implicitHeight: 40 * heightRate
                ...
                onPramLessonId: {
                    ...
                    mgrRemind.getRemindTag(idList);
                    navigation.keyWord = lessonId;
                    mgrRemind.getRemind(2,1);
                    navigation.setActiveView(1,0);
                }

                onTotalChanged: {
                    if(total > 0){
                        tipsView.visible = true;
                        navigation.messageCount = total;
                    }else{
    ...
    function showLoginWindow(){
        ...
        windowView.height = Screen.width / 1366 * 460;//height * 0.6 * heightRate//
        windowView.setX((Screen.width - windowView.width) / 2);
        windowView.setY((Screen.height - windowView.height) / 2);
        mainView.visible = false;
    }
    ...
    function addNavigation(){
        ...
        navigation.addView("课程表",
                           "qrc:/studenthomepage/YMHomePageView.qml",
