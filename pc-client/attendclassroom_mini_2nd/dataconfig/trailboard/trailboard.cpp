...

{
    ...
    connect(m_handler, SIGNAL(sigAuthChange(QString,int,int,int,int)), this, SIGNAL(sigAuthChange(QString,int,int,int,int)));
...
void TrailBoard::setEraserSize(double size)
{
    m_eraserSize = size;
}
...
void TrailBoard::operationClearScreen(int type, int pageNo, int totalNum)
{
    this->clearScreen(type,pageNo,totalNum);
}

void TrailBoard::clearScreen(int type,int pageNo,int totalNum)
{
    qDebug() << "===clearScreen===" << type << pageNo << totalNum;
    if(m_handler != NULL)
    {
        ...
        QString msg = m_handler->operationMsgTemplate(type, pageNo, totalNum, coursewareId);
...
void TrailBoard::parseTrail(QString command)
{
    ...
    if (err.error == QJsonParseError::NoError)
    {
        ...
        if (command == "trail")
        {
            ...
            double width = ((contentObj.take("width").toVariant().toLongLong()) / factor);
            ...
            this->drawBezier(pointFs, width * StudentData::gestance()->midHeight, QColor("#" + color), type == 1 ? 1 : 2); //1:"pencil"
        }
...
void TrailBoard::mousePressEvent(QMouseEvent *event)
{
    ...
    if (event->button() == Qt::LeftButton)
    {
        ...
        else//教鞭
        {
            ...
            {
                //发送教鞭命令
                QString message = m_handler->pointReqMsgTemplate(factorValX, factorValY);
                m_handler->sendLocalMessage(message, false, false); //fix: 教鞭不需要重绘画板, 不需要加入重发队列中, 直接发送至服务器
            }
        }
        m_pointCount = 0;
        emit sigMousePress();
    ...
    if(event->buttons() &Qt::LeftButton)
    {
        ...
        else//教鞭
        {
            ...
            m_pointCount++;
             ...
             if(m_pointCount == 5) //fix: 教鞭位置信息实时传输,每5个点传输一次
            {
                m_pointCount = 0;
...
void TrailBoard::deviceLoad()
{
    if(m_handler != NULL)
    {
        ...
        int cpu =qAbs((kernel + user - idle) * 100 / (kernel + user));
        emit sigCPURate(cpu);
...
void TrailBoard::sendResponderMsg(int types)
{
    if(NULL != m_handler)
    {
        QString message = m_handler->responderMsgTemplate(types,0);
