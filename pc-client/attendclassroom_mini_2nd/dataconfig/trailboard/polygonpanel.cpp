...
void PolygonPanel::setInitWindowType( int types )
{
    m_polygon = types ;
...
QString PolygonPanel::doneBtnClicked()
{
    ...
    {
        QJsonObject object;
        object.insert("x", QString::number(m_points.at(i).x() / this->width(), 'f', 6));
        object.insert("y", QString::number(m_points.at(i).y() / this->height(), 'f', 6));
        arr.append(object);
    }
    if (m_points.size() > 2)
    {
        QJsonObject object;
        object.insert("x", QString::number(m_points.at(0).x() / this->width(), 'f', 6));
        object.insert("y", QString::number(m_points.at(0).y() / this->height(), 'f', 6));
        arr.append(object);
    }
    QJsonObject obj;
    obj.insert("trail", arr);
    obj.insert("domain", QString("draw"));
    obj.insert("command", QString("polygon"));
    QJsonDocument doc;
    doc.setObject(obj);
