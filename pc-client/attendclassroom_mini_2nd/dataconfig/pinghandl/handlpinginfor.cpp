...
void HandlPingInfor::getCurrentIpNetValue(QString address, QString lost, QString delay)
{
    ...
    if(delay.contains("%"))
    {
        emit sigCurrentNetworkStatus(0, 0,lost.toInt());
        return;
    }
    int netValue = delay.toInt();
    if(netValue >= 0 && netValue <= 60)
    {
        emit sigCurrentNetworkStatus(3, netValue,lost.toInt());
    }
    if(netValue >= 61 && netValue <= 150)
    {
        emit sigCurrentNetworkStatus(2, netValue,lost.toInt());
    }
    if(netValue >= 151 && netValue <= 999)
    {
        emit sigCurrentNetworkStatus(1, netValue,lost.toInt());
    }
    if(delay == "-1")
    {
        emit sigCurrentNetworkStatus(0, 0,lost.toInt());
