...
//    QMessageBox::critical(NULL, "aaa", strAppFileName, QMessageBox::Ok, QMessageBox::Ok); //测试中文乱码
    ...
    QSharedMemory shared("attendclassroommini2nd"); //exe被复制粘贴以后, 还是只能运行一个
    if (shared.attach())
    {
        qDebug() << QString("attendclassroommini2nd has already been running, please exit and run again.") << __FILE__ << __LINE__;
        return 0;
    }
    shared.create(1);
    ...
    if (!QFile::exists(systemPublicFilePath + "miniAttendTemp.ini"))
    {
        qDebug() << QString("File: %1 does not existed") .arg(systemPublicFilePath + "miniAttendTemp.ini") << __FILE__ << __LINE__;
        return 0;
    }

    QFile file(systemPublicFilePath + "miniAttendTemp.ini");
    if(!file.open(QIODevice::ReadOnly))
    {
        qDebug() << QString("Open file: %1 failed.") .arg(systemPublicFilePath + "miniAttendTemp.ini") << __FILE__ << __LINE__;
        return 0;
    }
