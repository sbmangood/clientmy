...
public:
    ...
    //查询课堂轨迹
    Q_INVOKABLE QByteArray getLessonWriteTrail(QString lessonId);

public:
    //根据讲义解析所有栏目
    void analyzeItemInfo(long planId, QJsonArray itemsArrays);
    //下载文件
    QByteArray httpDownloadFile(const QString &url);
...
signals:
   ...
   void sigMiniHandoutInfo(QJsonArray dataArray);//讲义信息信号
