    ...
    */
    function setCurrentBeShowedView(showData,type,browseStatus,topicModel){
        ...
        if(type == 1){//单选题
            ...
            //截图模式
            if(browseStatus){
                singleChoiceQuestionView.setClipStatus(true);
            }
            return;
        }
        if(type == 2){//多选题
        ...
        if(type == 3){//判断题
            judgmentQuestionView.updateClipStatus(false);
    ...
    YMHomeworkWrittingBoard{
        ...
        onSigBeSavedGrapAnswer: {
            sigShowCorrectPage(imageUrl,1,imgWidth,imgHeight);
            console.log("=====YMHomeworkWrittingBoard::imageUrl====", imageUrl,imgWidth,imgHeight);
        }
    }
