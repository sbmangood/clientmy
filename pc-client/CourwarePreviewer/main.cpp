...
#include "./videovoice/iLiveSDK/videorender.h"
...
int main(int argc, char *argv[])
{
    ...
    //=============================================
    QSharedMemory shared("CourwarePreviewer");
    if (shared.attach())
    {
        qDebug() << QString("CourwarePreviewer has already been running, please exit and run again.") << __FILE__ << __LINE__;
        return 0;
    }
    shared.create(1);
   ...
   // QTextCodec::setCodecForLocale(QTextCodec::codecForName("UTF-8"));
    ...
    if (!QFile::exists(systemPublicFilePath + "courware.ini")) //"stutemp.ini")){
    {
        qDebug() << QString("File: %1 does not existed") .arg(systemPublicFilePath + "courware.ini") << __FILE__ << __LINE__;
        return 0;
    }

    QFile file(systemPublicFilePath + "courware.ini");
    if(!file.open(QIODevice::ReadOnly))
    {
        qDebug() << QString("Open file: %1 failed.") .arg(systemPublicFilePath + "courware.ini") << __FILE__ << __LINE__;
        return 0;
    }
    ...
    file.close();
    // QFile::remove(systemPublicFilePath +"courware.ini");
    ...
    //=======================================
    bool lessonIsFinished = false;//当前课程是否已经结束
    //qDebug()<< StudentData::gestance()->m_lessonStatus<<"StudentData::gestance()->m_lessonStatus";
    StudentData::gestance()->m_lessonStatus == "1" ? lessonIsFinished = true : lessonIsFinished = false;
    ...
    //判断用老课件形式还是新课件形式显示
    YMCloudClassManagerAdapter tempManager;
    tempManager.getLessonWriteTrail(YMUserBaseInformation::lessonId);

    bool currentIsOldVersion = YMUserBaseInformation::currentIsOldVersion;//默认新版本显示
    ...
    if(currentIsOldVersion)
    {
        fullHeight = (int) fixLength * 10;
    }else
    {
        fullHeight = (int) fixLength * 9;
    }
    //全屏的x，y坐标
    ...
    if(currentIsOldVersion)
    {
        midHeight = (int) midFixLength * 10;
    }else
    {
        midHeight = (int) midFixLength * 9;
    }
    //中间的x，y坐标
    ...
    StudentData::gestance()->midHeight = midHeight * 0.82;
    StudentData::gestance()->midWidth = midWidth * 0.82;
    StudentData::gestance()->fullWidth = fullWidth * 0.89;
    StudentData::gestance()->fullHeight = fullHeight * 0.89;
    ...
    //非全屏画布
    engine.rootContext()->setContextProperty("midWidth", midWidth * 0.82);
    engine.rootContext()->setContextProperty("midHeight", midHeight * 0.82);
    ...
    //
    engine.rootContext()->setContextProperty("lessonIsFinished", lessonIsFinished );
    engine.rootContext()->setContextProperty("currentIsOldVersion", currentIsOldVersion );
