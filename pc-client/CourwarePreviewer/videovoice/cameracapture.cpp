...
bool CameraCapture::present(const QVideoFrame &frame)
{
    ...
    //DebugLog::gestance()->log("image::size" + QString::number( frame.width()) + QString::number( frame.height()));
    ...
    image = image.scaled(640, 480, Qt::IgnoreAspectRatio, Qt::SmoothTransformation); //IgnoreAspectRatio
    image = image.convertToFormat(QImage::Format_RGB888);//Format_RGB888
    //转换为BGR888
