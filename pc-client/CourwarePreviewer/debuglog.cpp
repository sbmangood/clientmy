...
bool DebugLog::doUpgrade_LocalLogInfo_To_Server()
{
    ...
    qDebug() << "==strFileFulPath===" << strFileFulPath;
...
QString DebugLog::uploadLog_To_OSS(QString logPath)
{
    ...
    QString key = QString("appLog/%1/${%2}-${%3}_%4").arg(times.toString("yyyy/MM/dd")) .arg(StudentData::gestance()->m_lessonId) .arg("CourwarePreviewer") .arg(fileName);
    ...
    if(dataObject.value("result").toString().toLower() == "success")
    {
        ...
        qDebug() << "LoadInforMation::uploadQuestionImgOSS::success";
        return s_url;
    }
    else
    {
...
bool DebugLog::saveLogUrl(QString logUrl)
{
    ...
    //qDebug() << "===dataArrayAAA==="  << url << dataArray << maps;
...
QString DebugLog::doGetLessonID()
{
    ...
    QString strLessonID = StudentData::gestance()->m_lessonId.trimmed();
    if(strLessonID.length() <= 1)
    {
        strLessonID = YMUserBaseInformation::lessonId.trimmed();
        if(strLessonID.length() <= 1)
        {
            QDateTime times = QDateTime::currentDateTime();
            strLessonID = StudentData::gestance()->m_userName + "_" + times.toString("yyyy_MM_dd_hhmmss");
        }
    }

    qDebug() << "DebugLog::doGetLessonID: " << strLessonID << StudentData::gestance()->m_userName <<__LINE__;
    return strLessonID + "_" + StudentData::gestance()->m_userName;
}
