...
void HandlPingInfor::uploadSeverAddress()
{
    ...
    qDebug() << "YMCloudClassManagerAdapter::uploadSeverAddress url" <<url<< urls << __LINE__;
}
...
{
    //qDebug()<<"HandlPingInfor::onUploadAllWebAddressList=="<< QString::fromUtf8( reply->readAll())<<reply->error() ;
    if(reply->error() == QNetworkReply::NetworkError::NoError)
    {
...
void HandlPingInfor::onSigSendAddressLostDelayInfro(QMap<QString, QPair<QString, QString> > addressLostDelay)
{
    ...
    for(int i = 0 ; it != addressLostDelay.end() ; i++, it++)
    {
        ...
        {
            qDebug() << "=======break=======" << StudentData::gestance()->m_selectItemAddress;
            break;
        }
        QString items = it.key();
...
bool HandlPingInfor::getChannelSwitch(QString channelNumber)
{
    //StudentData::gestance()->functionSwitch.value("agora").toString() 的时候, "1"变成空字符串了
    //QJsonObject里面, value是double类型

    if(channelNumber == "1")
    {
        QString strAgora = StudentData::gestance()->functionSwitch.value("agora").toString();
        if(strAgora.trimmed().length() <= 0)
        {
            strAgora = QString::number(StudentData::gestance()->functionSwitch.value("agora").toDouble(), 10, 0);
        }

        qDebug() << "HandlPingInfor::getChannelSwitch strAgora" << strAgora << __LINE__;
        if("1" == strAgora)
        {
            return true;
        }
    ...
    {
        QString strTencent = StudentData::gestance()->functionSwitch.value("tencent").toString();
        if(strTencent.trimmed().length() <= 0)
        {
            strTencent = QString::number(StudentData::gestance()->functionSwitch.value("tencent").toDouble(), 10, 0);
        }

        qDebug() << "HandlPingInfor::getChannelSwitch strTencent" << strTencent << __LINE__;
        if("1" == strTencent)
        {
            return true;
        }
    ...
    {
        QString strWangyi = StudentData::gestance()->functionSwitch.value("wangyi").toString();
        if(strWangyi.trimmed().length() <= 0)
        {
            strWangyi = QString::number(StudentData::gestance()->functionSwitch.value("wangyi").toDouble(), 10, 0);
        }

        qDebug() << "HandlPingInfor::getChannelSwitch strWangyi" << strWangyi << __LINE__;
        if("1" == strWangyi)
        {
            return true;
        }
        ...
        {
            return false; //C通道, 现在服务器后台的开关, 还没有做, 为了方便测试, 这里先修改为开
            //            return true;
        }
    }
