...
class YMCloudClassManagerAdapter : public QObject
{
    ...
    public:
        ...
        //教室内改变讲义栏目接口
        Q_INVOKABLE void changeItem(long planId, long itemId);
    ...
    //作业获取
   Q_INVOKABLE QJsonObject getStuHomeWorkData(int pageIndex);

    //作业详情获取
  Q_INVOKABLE  QJsonObject getStuHomeWorkDetailData(QString lessonWorkId);
    ...
    signals:
        ...
        void sigLessonCommentConfig(QJsonArray dataArray);//课程结束时评价的配置

        void sigHomeWorkList(QJsonObject objData);//更新课后作业数据
        void sigHomeWorkDetail(QJsonObject objData);//更新课后作业详情
};
