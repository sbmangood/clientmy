...
YMHomeworkWrittingBoard::YMHomeworkWrittingBoard(QQuickPaintedItem *parent): QQuickPaintedItem(parent)
{
    ...
    if(YMUserBaseInformation::currentDateTimes == 0)
    {
        YMUserBaseInformation::currentDateTimes = QDateTime::currentDateTime().toTime_t();
    }
}
//设置画笔颜色
...
void YMHomeworkWrittingBoard::grapItemImage(QObject *itemObj)
{
    ...
    connect(grabResult, SIGNAL(ready()), &loop, SLOT(quit()));
    loop.exec();

    saveImage();
}

void YMHomeworkWrittingBoard::saveImage()
{
    ...
    QString names = QString("/%1.jpg").arg(++YMUserBaseInformation::currentDateTimes);
    qDebug() << "==YMHomeworkWrittingBoard::saveIamge==" << YMUserBaseInformation::currentDateTimes << m_docParh + names;
