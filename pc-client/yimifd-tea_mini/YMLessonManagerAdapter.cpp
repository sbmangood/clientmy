...
void YMLessonManagerAdapter::getCloudServer()
{
    ...
    QJsonObject dataObject = QJsonDocument::fromJson(dataArray).object();
    //qDebug()<<"==YMLessonManagerAdapter::getCloudServer== "<< dataObject;
    if(dataObject.contains("data"))
    {
        ...
        //qDebug() << "==join::classRomm::data==" << dataObj;
        ...
        else if(type == 1)
        {
            ...
            QString IpAddress = des_decrypt(QByteArray::fromHex(data).toStdString());//"123.206.203.83,100";//
...
void YMLessonManagerAdapter::enterListen()
{
    ...
    programRuned();
    connect(enterRoomprocess, SIGNAL(finished(int)), this, SIGNAL(lessonlistRenewSignal(int)));
}
...
void YMLessonManagerAdapter::getJoinClassRoomInfo(QString executionPlanId)
{
    ...
    QJsonObject dataObj = QJsonDocument::fromJson(dataArray).object();
    qDebug()<<"YMLessonManagerAdapter::getJoinClassRoomInfo("<< executionPlanId << dataObj << url << YMUserBaseInformation::token;
    if(dataObj.value("result").toString() == "success")
    {
        ...
        if(platform == 1)//拓课云
        {
            emit sigJoinClassroom(liveRoomObj);
        }
        //进入教室
        //enterClass();
    }
...
void YMLessonManagerAdapter::getListenCloudServerIp()
{
    ...
    QJsonObject dataObject = QJsonDocument::fromJson(dataArray).object();
    qDebug()<<"getCloudServer data "<< dataObject;
    if(dataObject.contains("data"))
    {
        QJsonObject dataObj = dataObject.value("data").toObject();
        m_ipAddress = dataObj.value("domain").toString();
