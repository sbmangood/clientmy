...
{
    QVariantMap reqParam;
    reqParam.insert("appName","tea_pc");//YMUserBaseInformation::type);
...
{
    //qDebug()<< "===YMAccountManager::findLogin==" << username << password;
    QVariantMap reqParam;
    reqParam.insert("loginName",username);
...
//    reqParam.insert("userType",1);
//    reqParam.insert("operExt","");
//    reqParam.insert("identifyCode","");

//    QString sortPram = YMEncryption::signMapSort(reqParam);
//    QString signKey = YMEncryption::md5(sortPram);

//    reqParam.insert("signKey",signKey);

    //QString url = m_httpClient->httpUrl + "/login";
...
//    reqParam.insert("signKey",signKey);
    ...
    QByteArray dataArray = m_httpClient->httpPostVariantHanlder(url , reqParam);
    QJsonObject dataObj = QJsonDocument::fromJson(dataArray).object();
    qDebug() << "==333333333333==" << dataObj << url<< dataArray;
    if(dataObj.value("message").toString() == QStringLiteral("成功"))
    {
        qDebug() << "******message********" << dataObj.value("message").toString();
        ...
        //emit sigTalkLoginInfo(dataObject);
    ...
    //qDebug() << "==YMAccountManager::talkLogin==" << url << dataObj << YMUserBaseInformation::token;
}
...
void YMAccountManager::talkGetUserInfo()
{
    ...
    //qDebug() << "==YMAccountManager::talkGetUserInfo==" << dataObj;
    //emit sigTalkTeacherInfo(dataObj);
}
