...
void WhiteBoard::setWhiteBoardCallBack(IWhiteBoardCallBack* whiteBoardCallBack)
{
    ...
    if(m_whiteBoardCallBack)
        m_whiteBoardCallBack->onCurrentWhiteBoardSize(this->width(), this->height());
}

void WhiteBoard::setUserAuth(const QString &userId, int userRole, int trailState)
{
    emit sigAuthChange(userId, 0, trailState, userRole);
}
