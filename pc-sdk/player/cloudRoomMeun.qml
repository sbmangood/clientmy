...
Item {
    ...
    property bool disableButton: false;//禁用菜单点击
    ...
    signal sigExplainMode(var itemId,var planId,var questionId,var itemType);
    signal sigLoadingSuccess();//加载完成
    signal sigSendMenuName(var menuName);//当前菜单栏名称信号
...
//        }
    ...
    function updateSelected(planId,itemId){
        //console.log("=====11updateSelected11=====",planId,itemId);
        ...
        for(var i = 0; i < menuModel.count; i++){
            ...
            if(planIds == planId && itemId == itemsId){
                menuModel.get(i).selected = true;
                sigSendMenuName(menuModel.get(i).itemName);
                continue;
            }
