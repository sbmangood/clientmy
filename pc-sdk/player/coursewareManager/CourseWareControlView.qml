...
//课件显示层控制中心  图片课件 h5课件 结构化课件
Item {
    anchors.fill: parent
    property int currentBeshowViewType: 0;// 当前被显示的课件界面类型 0空白界面 1图片课件界面 2h5课件 3结构化课件
    signal sigImageLoadReadys();// 图像加载状态未ready
    signal sigChangeScoreBarVisibles(var visibles);// 设置滚动条的显示状态
    signal sigUploadLoadingImgFailLog();// 图片加载失败信号
...
*/
    ...
    function coursewareOperation(coursewareType, operationType, operationIndex, step){
        // 目前只有H5课件需要此接口
        console.log("==coursewareOperation====",operationType,step)
        if(coursewareType == 3){
     ...
     * H5课件激活与禁用接口
     ****************************/
    function updateH5EngineView(isEnable){
        h5EngineView.enabled = isEnable;
    }
}
