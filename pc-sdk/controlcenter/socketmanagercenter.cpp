...
void SocketManagerCenter::initSocketMessageCtrl(const QVariantList &goodIpList, const QString &address, int port, int serverDelay, int serverLost)
{
...
bool SocketManagerCenter::checkServerResponse(QString target, bool bConfirmFinish)
{
    ...
    if(nullptr != m_socketMessageCtrl)
    {
        return m_socketMessageCtrl->checkServerResponse(target, bConfirmFinish);
    }
    else
    {
...
void SocketManagerCenter::onPersonData(const QString &address, int port)
{
