...
AudioVideoCenter::AudioVideoCenter(int userRole): m_IAudioVideoCtrl(nullptr), m_userRole(userRole)
{
...
void AudioVideoCenter::init(const QString &pluginPathName)
{
    if(pluginPathName.endsWith("YMAudioVideoManager.dll", Qt::CaseInsensitive))
    {
        ...
        if(instance)
        {
            ...
            connect(m_IAudioVideoCtrl, SIGNAL(sigAudioVolumeIndication(uint,int)), this, SIGNAL(sigAudioVolumeIndication(uint,int)));
...
void AudioVideoCenter::changeChanncel()// 切换频道
{
    ...
    else if(m_userRole == 1)
    {
        DataCenter::getInstance()->m_role = STUDENT;
    }
    else if(m_userRole == 2)
    {
        DataCenter::getInstance()->m_role = AUDITOR;
    }
...
//            <<",m_channelKey="<<m_channelKey<<",m_channelName="<<m_channelName;
    ...
    if(NULL != m_IAudioVideoCtrl)
    {
        m_IAudioVideoCtrl->changeChanncel(currentChannel, DataCenter::getInstance()->m_videoType, DataCenter::getInstance()->m_microphoneState, DataCenter::getInstance()->m_cameraState,
                                          ...
                                          videoSpan, DataCenter::getInstance()->m_channelKey, DataCenter::getInstance()->m_channelName, BIG_CLASS);
    }
}
...
int AudioVideoCenter::setUserRole(CLIENT_ROLE role)// 设置用户角色
{
    int ret = -1;
    if(NULL != m_IAudioVideoCtrl)
    {
        ret = m_IAudioVideoCtrl->setUserRole(role);
    }
    return ret;
}

int AudioVideoCenter::setVideoResolution(VIDEO_RESOLUTION resolution)// 设置视频分辨率
{
    int ret = -1;
    if(NULL != m_IAudioVideoCtrl)
    {
        ret = m_IAudioVideoCtrl->setVideoResolution(resolution);
    }
    return ret;
}
...
//        }
//    }
     emit sigJoinroom(uid, QString::number(uid), status);
}
