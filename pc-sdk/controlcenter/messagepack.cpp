...
QString MessagePack::enterRoomReqMsg(bool &bServerResp, bool &bConfirmFinish)
{
    ...
    QJsonObject content;
     ...
     //协议中的content内容
    content.insert(kSocketAppId, DataCenter::getInstance()->m_appId);
    content.insert(kSocketBusinessType, DataCenter::getInstance()->m_classroomType);
    content.insert(kSocketGroupId, StudentData::gestance()->m_groupId);
    ...
    QString userRole = DataCenter::getInstance()->m_userRole == 0 ? "TEA" : (DataCenter::getInstance()->m_userRole == 2 ? "AST" : "STU");
    content.insert(kSocketUserType, userRole);
    content.insert(kSocketPlat, kSocketTPC);

    QJsonObject extraInfoObj;
    extraInfoObj.insert("videoId",StudentData::gestance()->m_agoraUid);
    extraInfoObj.insert("userName", DataCenter::getInstance()->m_nickName);
    extraInfoObj.insert("systemInfo",StudentData::gestance()->m_sysInfo);
    QJsonDocument doc(extraInfoObj);
    QString extraInfoStr = QString(doc.toJson(QJsonDocument::Compact));
    content.insert(kSocketExtraInfo,extraInfoStr);
...
QString MessagePack::sendRedPacketsMsg(const QJsonObject &content)
{
    quint64 lid = StudentData::gestance()->m_liveRoomId.toLongLong();
    quint64 uid = StudentData::gestance()->m_currentUserId.toLongLong();

    QString msg = createMessageTemplate(kSocketSendRedPackets, lid, uid, content);
    return msg;
}

QString MessagePack::hitRedPacketMsg(const QJsonObject &content)
{
    quint64 lid = StudentData::gestance()->m_liveRoomId.toLongLong();
    quint64 uid = StudentData::gestance()->m_currentUserId.toLongLong();

    QString msg = createMessageTemplate(kSocketHitRedPacket, lid, uid, content);
    return msg;
}

QString MessagePack::questionMsg(const QJsonObject &content)
{
    quint64 lid = StudentData::gestance()->m_liveRoomId.toLongLong();
    quint64 uid = StudentData::gestance()->m_currentUserId.toLongLong();

    QString msg = createMessageTemplate(kSocketQuestion, lid, uid, content);
    return msg;
}
...
QString MessagePack::createMessageTemplate(QString command, quint64 lid, quint64 uid, QJsonObject content)
{
    ...
    if(kSocketTrail == command || kSocketAck == command || kSocketEnterRoom == command || kSocketHb == command || kSocketSynFin == command || kSocketPoint == command)
    {
        QMutexLocker locker(&m_mMessageNumMutex);
    ...
    if(MINI_CLASSROOM == DataCenter::getInstance()->m_classroomType)
    {
        messageTemplate[kSocketVer] = COMMUNICATION_PROTOCOL_VER;
    }
    else if(BIG_CLASSROOM == DataCenter::getInstance()->m_classroomType)
    {
        messageTemplate[kSocketVer] = BIGCLASSROOM_PROTOCOL_VER;
    }
    else
    {
        messageTemplate[kSocketVer] = COMMUNICATION_PROTOCOL_VER;
