...
void CoursewareCenter::goPage(int pageIndex)
{
    ...
    if(DataCenter::getInstance()->m_currentDocType == 3)// H5
    {
        QJsonObject userInfo;
        int userRole = tea_role;
        if(NULL != m_controlCenter)
        {
            userInfo = m_controlCenter->getUserInfo();
        }
        if(userInfo.contains("userRole"))
        {
            userRole = userInfo.take("userRole").toInt();
        }
        //qDebug()<<"=======CoursewareCenter::goPage::userRole======="<<userRole;
        if(userRole != tea_role)
        {
            if(NULL != m_YMCoursewareManager)
            {
                m_YMCoursewareManager->sendSigH5GoPage(pageIndex);
            }
        }
    }
    else
            ...
            if (DataCenter::getInstance()->m_pages.contains(DataCenter::getInstance()->m_currentCourse))
            {
                if (DataCenter::getInstance()->m_pages[DataCenter::getInstance()->m_currentCourse].size() > 1)
                {
                    ...
                    if (DataCenter::getInstance()->m_sysnStatus)
                    {
                        if(m_YMCoursewareManager)
                        {
                            qDebug()<<"==========CoursewareCenter::sendSigCurrentQuestionId=";
...
void CoursewareCenter::addPage()
{
    ...
    if(DataCenter::getInstance()->m_currentDocType == 3)// H5
    {
        ...
        if(userRole != tea_role)
        {
            if(NULL != m_YMCoursewareManager)
            {
                m_YMCoursewareManager->sendSigH5AddPage();
            }
        }
    }
}
...
void CoursewareCenter::delPage()
{
    ...
    if(DataCenter::getInstance()->m_currentDocType == 3)// H5
    {
        ...
        if(userRole != tea_role)
        {
            if(NULL != m_YMCoursewareManager)
            {
                m_YMCoursewareManager->sendSigH5DelPage();
            }
        }
    }
...
void CoursewareCenter::setPlayAnimation(QString pageId, int step)
{
    if(m_YMCoursewareManager)
    {
        m_YMCoursewareManager->sendSigPlayAnimation(pageId, step);
    }
}

void CoursewareCenter::parseAnimationInfo(int pageNo, int step)
{
    if(m_YMCoursewareManager)
    {
        m_YMCoursewareManager->sendSigAnimationInfo(pageNo, step);
    }
}
