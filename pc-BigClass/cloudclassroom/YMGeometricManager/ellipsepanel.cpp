...
void EllipsePanel::setInitWindowType(QString boardId,QString dockId,QString itemId,int pageId,double lineWidth)
{
    m_boardId = boardId;
    m_dockId = dockId;
    m_pageId = pageId;
...
void EllipsePanel::paint(QPainter *painter)
{
    ...
    if(m_isStretch)
    {
        ...
        switch(m_stretchState)
        {
            case TOP:
            {
                centerPoint = QPoint(m_topStretch.x() - m_topStretch.width(), m_topStretch.y() - m_topStretch.height());
                rotatePoint = pointAfterRotate(centerPoint, 180);
                qDebug() << "===TOP===" << centerPoint;
            }
            break;
            case LEFT:
            {
...
QString  EllipsePanel::doneBtnClicked()
{
    ...
    QJsonObject obj;
    int locationX = m_rectX / this->width() * factor;
    int locationY = m_rectY / this->height() * factor;
    int rectWidth = m_rectWidth / this->width() * factor;
    int rectHeight = m_rectHeight / this->height() * factor;
    obj.insert("rectX", QString::number(locationX));
    obj.insert("rectY", QString::number(locationY));
    obj.insert("rectWidth", QString::number(rectWidth));
    obj.insert("rectHeight", QString::number(rectHeight));
    obj.insert("boardId", m_boardId);
    obj.insert("itemId", m_itemId);
    obj.insert("color","3ED7B7");
    obj.insert("dockId",m_dockId);
    obj.insert("width", m_lineWidth * factor);
    obj.insert("pageId",m_pageId);
    obj.insert("type", QString("ellipse"));
