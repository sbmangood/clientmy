...
public:
    ...
    //设置鼠标形状
    Q_INVOKABLE void selectShape(int shapeType, const QString &id);
    //设置画笔尺寸
    Q_INVOKABLE void setPaintSize(double size, const QString &id);
    //设置画笔颜色
    Q_INVOKABLE void setPaintColor(int color, const QString &id);
    //设置橡皮大小
    Q_INVOKABLE void setErasersSize(double size, const QString &id);

    //回撤轨迹数据
    Q_INVOKABLE void undoTrail(const QString &id);
    //清除多个轨迹数据
    Q_INVOKABLE void clearTrails(const QString &id);
    // 跳转白板页,type 1翻页 2加页 3减页
    Q_INVOKABLE void goWhiteBoardPage(int type,int pageNo,int totalPage, const QString &whiteBoardId, bool bSendMsg = true);

    Q_INVOKABLE void uninit();

    // 加载课件
    Q_INVOKABLE void insertCourseWare(QJsonArray imgUrlList, QString fileId,QString h5Url,int coursewareType, QString docName);
    ...
    // 课件窗口操作
    Q_INVOKABLE void coursewareWindowUpdate(QString coursewareId, QString type, double recWidth, double recHeight, double recX, double recY);
    ...
    // 音视频课件
    Q_INVOKABLE void setAVCourseware(QString types, QString state, QString times, QString address, QString fileId, QString docName, QString suffix);
    ...
    Q_INVOKABLE void sendDrawGraph(QJsonObject graphObj);//发送几何图形数据
    Q_INVOKABLE void sendUploadImage(QJsonObject imageMsgObj);
    Q_INVOKABLE void deleteItem(const QString &boardId,const QString &itemId);
    Q_INVOKABLE void addFeedbackInfo(const QString &feedbackInfo, const QString & envType, const QString & className, const QString &userName, const QString &classId, const QString &userRole);    //添加报障信息
...
signals:
    ...
    void sigCurrentCoursewareId(QString currentCoursewareId);// 当前课件Id
    ...
    void sigWhiteBoardPages(int curPage, int totalPage, QString id);
    void sigDrawGraph(QJsonObject graphObjData);
    void sigClearTrails(QString boardId);
    void sigUserAuth(QString userId,int up,int trail,int audio,int video,bool isSynStatus);
    void sigDeleteItem(QString boardId,QString itemId);
    void sigLoadClassComplete(QString stateCode);    //加载教室完成
