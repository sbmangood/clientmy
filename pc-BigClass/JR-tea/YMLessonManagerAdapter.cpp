...
void YMLessonManagerAdapter::getTeachLessonInfo(QString dateTime)
{
    ...
    QString strDomainName = "https://omo-upc-api-dev.onesmart.org/api/Lessons/daily?";

    QByteArray byteArray = m_httpClint->httpGetData( strDomainName,YMUserBaseInformation::token);
    QJsonObject dataObject = QJsonDocument::fromJson(byteArray).object();

    teachLessonInfoChanged(dataObject);
...
void YMLessonManagerAdapter::runClassRoom(QJsonObject roomData)
{
    ...
    tempData.append("&userId=").append( QString::number( roomData.value("userId").toInt()).toStdString().c_str());
...
void YMLessonManagerAdapter::runCourse(QJsonObject roomData)
{
    QString runPath = QCoreApplication::applicationDirPath();
    QStringList courseData;
    courseData.append(roomData.value("roomId").toString());
    courseData.append(roomData.value("appId").toString());
    courseData.append(roomData.value("appKey").toString());
    courseData.append(roomData.value("envType").toString());
    qDebug()<<"YMLessonManagerAdapter::runCourse"<<roomData;
...
{
    QString strDomainName = "https://omo-upc-api-dev.onesmart.org/api/Lessons/daily?date=" + dayData;
    if( m_httpClint->m_stage == "api")
    {
        strDomainName = strDomainName.remove("-dev");
    }
    ...
    httpRequest.setHeader(QNetworkRequest::ContentTypeHeader, "application/x-www-form-urlencoded");
    QString headerData = "Bearer " + YMUserBaseInformation::token;
    httpRequest.setRawHeader("Authorization",headerData.toLocal8Bit());
    QSslConfiguration conf = httpRequest.sslConfiguration();
    conf.setPeerVerifyMode(QSslSocket::VerifyNone);
    conf.setProtocol(QSsl::TlsV1SslV3);
    httpRequest.setSslConfiguration(conf);
    netWorkMgr->get(httpRequest);
...
{
    QString strDomainName = "https://omo-upc-api-dev.onesmart.org/api/Lessons/dates?startDate=" + startDay +"&endDate=" + endDay ;
    if( m_httpClint->m_stage == "api")
    {
        strDomainName = strDomainName.remove("-dev");
    }
...
void YMLessonManagerAdapter::onGetMonthLessonDataFinish(QNetworkReply *reply)
{
    ...
    if(dataObject.value("status").toInt() == 0)
    {
    ...
    {
        QString tempData = lessonArry.at(a).toString();
        lessonArry.replace(a, QJsonValue(tempData.replace("T00:00:00","")));
    }

    qDebug() << "YMLessonManagerAdapter::getMonthLessonData" << lessonArry.size()<<lessonArry;
    sigGetMonthLessonData(lessonArry);
    reply->deleteLater();
}
