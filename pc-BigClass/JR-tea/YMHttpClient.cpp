...
QString YMHttpClient::getRunUrl(int urlType)
{
    ...
    if(environmentType == 1) //H5的页面
    {
        YMUserBaseInformation::m_strWriteListenUrl = YMUserBaseInformation::m_strHttpHead_Test + strFreeTrial;
    }
    else
    {
        YMUserBaseInformation::m_strWriteListenUrl= strHttpHeader + strFreeTrial;
    }
    ...
    if(environmentType == 1) //H5的页面
    {
        YMUserBaseInformation::m_strListenUrl = YMUserBaseInformation::m_strHttpHead_Test + strFreeTrialResult;
    }
    else
    {
...
void YMHttpClient::updateNetType(int netType, QString stage)
{
    ...
    if(netType == 1)
    {
        ...
        if(isPublicTest)
        {
            m_setting->setValue("V2.4/formal", "test-api.yimifudao.com/v2.4");
            m_setting->setValue("Study/formal", "test-jyhd.yimifudao.com");
        }else
        {
  ...
  QByteArray YMHttpClient::httpPost(const QString &url,const QByteArray &data)
{

    const QUrl aurl(url);
    QNetworkAccessManager * netWorkMgr = new QNetworkAccessManager();
    QNetworkRequest httpRequest;
    httpRequest.setUrl(aurl);
    ...
    httpRequest.setSslConfiguration(conf);
    QEventLoop httploop;
    QNetworkReply * reply = netWorkMgr->post(httpRequest,data);
    connect(reply, SIGNAL(finished()), &httploop, SLOT(quit()));
    httploop.exec();
   ...
   QByteArray YMHttpClient::httpGetData(const QString &url,const QString &token)
   {
       ...
       httpRequest.setSslConfiguration(conf);
       QEventLoop httploop;
       QNetworkReply * reply = netWorkMgr->get(httpRequest);
       connect(reply, SIGNAL(finished()), &httploop, SLOT(quit()));
       httploop.exec();
