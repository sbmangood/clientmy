...
#include "debuglog.h"
#include "../jwtcpp/base.h"
#include "../jwtcpp/jwt.h"
#include "../jwtcpp/picojson.h"
...
void YMAccountManager::login(const QString &username, QString &password)
{


    QString strDomainName = "https://omo-upc-api-dev.onesmart.org/api/Teachers/login";
    if( m_httpClient->m_stage == "api")
    {
        strDomainName = strDomainName.remove("-dev");
    }
    QJsonObject msgObj;
    msgObj.insert("hrCode",username);
    msgObj.insert("password",YMEncryption::md5(password));
    QJsonDocument document=QJsonDocument(msgObj);
    QByteArray arrayData = document.toJson();
    QByteArray byteArray = m_httpClient->httpPost(strDomainName ,arrayData);
    ...
    if(dataObject.contains("status")
            && dataObject.value("status").toInt() == 0){
        //解析消息
        QJsonObject teacherObj = dataObject.value("data").toObject();
        auto decoded = jwt::decode(teacherObj.value("accessToken").toString().toStdString());
        std::string  strkey ("TeacherName");
        auto strName  =  decoded.get_payload_claim(strkey).to_json().to_str();
        YMUserBaseInformation::realName = QString::fromStdString(strName);
        YMUserBaseInformation::token = teacherObj.value("accessToken").toString();
