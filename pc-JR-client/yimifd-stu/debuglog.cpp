...
static QString g_strAppFullPath_LogFile;
...
void DebugLog::init_log(QString strAppFileName)
{
    ...
    //得到log文件的绝对路径
    QString strAppFullPath = g_strAppFullPath;
    strLogFile = strAppFullPath.replace(g_strAppFileName, strLogFile); //得到log文件的绝对路径
    g_strAppFullPath_LogFile = strLogFile;
...
bool DebugLog::saveLogUrl(QString logUrl)
{
    ...
    maps.insert("lessonId", doGetLessonID());
    ...
    QString url = m_httpClient->httpUrl + "/log/saveLogUrl";
...
QString DebugLog::uploadLog_To_OSS(QString logPath)
{
    ...
    QString key = QString("appLog/%1/${%2}-${%3}_%4") .arg(times.toString("yyyy/MM/dd")) .arg("0").arg("yimifd-stu") .arg(fileName);
...
bool DebugLog::doUpgrade_LocalLogInfo_To_Server()
{
    ...
    //====================================
    //上传当前工程的日志文件
    QString strFileFulPath = g_strAppFullPath_LogFile;
...
bool DebugLog::doUpgrade_LocalLogInfo_To_Server_Once(QString strFileFulPath)
{
    ...
    maps.insert("lessonId", doGetLessonID()); //主教室, 还没有进入没有lesson id, 默认填0
    maps.insert("appVersion", YMUserBaseInformation::appVersion);
    ...
    //logUpload接口, 参考文档: 一米辅导API接口文档（V2.4）.doc
//    QString httpsd = "http://api.yimifudao.com.cn/v2.4/log/logUpload?" + QString("%1").arg(urls);
    QString httpsd = m_httpClient->httpUrl + "/log/logUpload?" + QString("%1").arg(urls);
...
//从stutemp.ini文件中, 获取lesson id
QString DebugLog::doGetLessonID()
{
    //lessonId 的默认值是: 登录名 + 时间
    QDateTime times = QDateTime::currentDateTime();
    QString strLessonID = YMUserBaseInformation::userName + "_" + times.toString("yyyy_MM_dd_hhmmss");
    qDebug()<<"DebugLog::doGetLessonID"<<strLessonID;
    ...
    if (!QFile::exists(systemPublicFilePath + "stutemp.ini")) //"stutemp.ini")){
    {
        qDebug() << QString("!QFile.exists") << systemPublicFilePath + "temp.ini" << __LINE__;
        return strLessonID;
    }

    QFile file(systemPublicFilePath + "stutemp.ini"); //"stutemp.ini");
    ...
    {
        qDebug() << QString("backDatas.length() < 1") << __LINE__;
        return strLessonID;
    }
    ...
    if(error.error == QJsonParseError::NoError)
    {
        if(documet.isObject())
        {
            ...
            if(jsonObjs.contains("data"))
            {
                ...
                strLessonID = QString("%1").arg( jsonObj.take("lessonId").toInt() );
            }
        }
    }

    //qDebug() << "DebugLog::doGetLessonID: " << strLessonID << YMUserBaseInformation::userName <<__LINE__;
    return strLessonID + "_" + YMUserBaseInformation::userName;
}
