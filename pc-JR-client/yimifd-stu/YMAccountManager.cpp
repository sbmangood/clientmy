...
void YMAccountManager::login(QString username, QString password)
{
    ...
    reqParam.insert("umengAppKey", "1");
    reqParam.insert("appSource", "YIMI");
    reqParam.insert("loginType", 0);
    QByteArray dataArray = m_httpClient->httpPostForm(m_httpClient->httpUrl + "/user/userLogin", reqParam);
    QJsonObject dataObject = QJsonDocument::fromJson(dataArray).object();

    qDebug() << "YMAccountManager::login: " << dataObject << dataObject.contains("result");
    if(dataObject.contains("code") && dataObject.value("code").toString().toLower() == "00000")
    {

        QJsonObject teacherObj = dataObject.value("data").toObject();
        YMUserBaseInformation::token = teacherObj.value("token").toString();
        QJsonObject dataObj= getApiUserInfo(YMUserBaseInformation::token);
        QJsonObject stuObj = dataObj.value("data").toObject();

        YMUserBaseInformation::realName = stuObj.value("realName").toString();
        YMUserBaseInformation::nickName = stuObj.value("nickName").toString();
        YMUserBaseInformation::headPicture = stuObj.value("headUrl").toString();
        YMUserBaseInformation::MD5Pwd = YMEncryption::md5(password);
        YMUserBaseInformation::id = QString::number(stuObj.value("userId").toInt());
        YMUserBaseInformation::mobileNo = stuObj.value("mobileNo").toString();
        ...
        int id = stuObj.value("userId").toInt();
...
void YMAccountManager::getLatestVersion()
{
    ...
    if(!YMUserBaseInformation::m_bIsPublicTest)
    {
        ...
        reqParam.insert("appName", "stu_pc_1v1"); //生产版本
    }
    else
    {
...
{
    return m_firmware.value("downloadUrl").toString();
}

//YMUserBaseInformation::id

void YMAccountManager::getMiniClassToken()
{
    QString strDomainName = "http://liveroom.yimifudao.com.cn/v1.0.0/openapi/sts/getToken";
    if( m_httpClient->m_stage != "api")
    {
        strDomainName = strDomainName.replace("liveroom",m_httpClient->m_stage + "-liveroom");
    }
    AESCryptManager aesManager(QString("a995732df99bc794").toStdString());
    QString aesCert = QString::fromStdString(aesManager.EncryptionAES(QString("a995732df99bc794").toStdString()));

    QString postData = QString("{\"appid\": \"3edaf2877c994a4e8739dbe34411e7d7\",\"certificate\": \"%2\",\"userId\": \"%1\",\"userType\": 1}").arg(YMUserBaseInformation::id).arg(aesCert);//
    qDebug()<<"YMAccountManager::getMiniClassToken()"<<m_httpClient->m_stage<<strDomainName<<postData;
    ...
    connect(netWorkMgr, SIGNAL(finished(QNetworkReply*)), this, SLOT(onGetMiniClassToken(QNetworkReply*)));
}

void YMAccountManager::onGetMiniClassToken(QNetworkReply *reply)
{
    QByteArray byteArray = reply->readAll();
    QJsonObject dataObject = QJsonDocument::fromJson(byteArray).object();
    YMUserBaseInformation::miniClassToken = dataObject.value("data").toObject().value("token").toString();
    qDebug() << "YMAccountManager::onGetMiniClassToken" <<YMUserBaseInformation::miniClassToken<< dataObject << byteArray;
    reply->deleteLater();
}


QString YMAccountManager::getCurrentStage()
{
    return m_httpClient->m_stage;
}

QJsonObject YMAccountManager:: getApiUserInfo(const QString &token)
{

    QVariantMap reqParam;
    reqParam.insert("token", token);
    QString url = m_httpClient->httpUrl + "/user/getUserInfoByToken";
    QByteArray dataArray = m_httpClient->httpPostForm(url, reqParam);
    QJsonObject dataObject = QJsonDocument::fromJson(dataArray).object();
      qDebug() << "YMAccountManager::getApiUserInfo" << dataObject;
    return dataObject;
}
