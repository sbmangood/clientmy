...
MouseArea {
    ...
    YMLessonManagerAdapter{
        ...
        onSigMessageBoxInfo:
        {
            console.log("=======YMResearchView.qml onSigMessageBoxInfo========");
        ...
        onSigUserSubjectInfo: {
            ...
            var gradeObj = lessonMgr.getGrades();
            var gradeItems = gradeObj.data;
            gradeModel.clear();
...
//                                    enterClassRoom = false;  //"进入教室"按钮, 一直都可以点击, 因为在classroom程序中, 已经控制了, 只有一个实例
                                    if(teacherId == userId){
                                        ...
                                        console.log("====enterClassRoom22=====", lessonId)
    ...
    function disableClassButton(remaining,startTime,endTime,index){
        ...
        if((endDate.getTime()-currentDate.getTime()) / 1000 / 60 > -60 && status == 0)
        {
            teachModel.get(index).enableClass = true;
            return;
        }
