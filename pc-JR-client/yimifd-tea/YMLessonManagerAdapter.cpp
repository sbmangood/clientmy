    ...
    ,offSingle(1)
{
    ...
    m_CloudhttpClient = new HttpClient();

    QString envType = YMUserBaseInformation::envType == "api" ? "" : YMUserBaseInformation::envType + "-";
    m_apiUrl = "http://" +  envType+ "liveroom.yimifudao.com/v1.0.0/openapi";
}
...
void YMLessonManagerAdapter::getTeachLessonListInfo(QJsonObject data)
{
    ...
    dataMap.insert("queryDate", queryStartDate);
    ...
    QString url = m_httpClint->httpUrl + "/oneToMore/lesson/teacherLessonList";

    QByteArray dataArray = m_httpClint->httpPostForm(url, dataMap);
    //qDebug()<<dataArray <<"lessonlist data";
    QJsonObject objectData = QJsonDocument::fromJson(dataArray).object();
    qDebug() << "YMLessonManagerAdapter::getTeachLessonListInfo" <<url << dataMap<< objectData;
...
void YMLessonManagerAdapter::downLoadFile(QJsonObject dataObject, QString fileDir)
{
    ...
    QProcess::startDetached(runPath + "/playerOld.exe", courseData);
}
...
void YMLessonManagerAdapter::runClassRoom(QJsonObject roomData)
{
    ...
    qDebug()<<"YMLessonManagerAdapter::runClassRoom"<<roomData<<courseData;
    connect(enterRoomprocess, SIGNAL(finished(int)), this, SIGNAL(lessonlistRenewSignal()));
}
//查看新录播
...
void YMLessonManagerAdapter::getCurrentMonthLessonData(QString currentMonth)
{
    ...
    QString url = m_httpClint->httpUrl + "/oneToMore/lesson/teacherLessonSchedule";
...
void YMLessonManagerAdapter::getCurrentDayLessonData(QString currentDay)
{
    ...
    dataMap.insert("queryDate", currentDay);
    //dataMap.insert("type", YMUserBaseInformation::type);
    ...
    QString url = m_httpClint->httpUrl + "/oneToMore/lesson/teacherLessons";
...
void YMLessonManagerAdapter::onGetCurrentDayLessonDataFinish(QNetworkReply *reply)
{
    ...
    for(int a = 0; a < arrayData.size(); a++)
    {
        ...
        if(lessonTime.split("-").size() == 2)
        {
            ...
            QJsonArray stuArray = arrayData[a].toObject().value("stuList").toArray();

            QString showNameText = "";
            for(int a = 0; a< stuArray.size();a++)
            {
                showNameText.append(stuArray.at(a).toObject().value("clientName").toString());
                if(a != stuArray.size() - 1)
                {
                    showNameText.append(QStringLiteral("、"));
                }
            }
            tObj.insert("childName",showNameText);
            arrayData[a] = tObj;
        }
    }
    qDebug()<<"getCurrentDayLessonData"<<arrayData;
    sigGetDayLessonData(arrayData);
}

int YMLessonManagerAdapter::getCloudDiskList(QString roomId, QString apiUrl, QString appId, bool isRefreshCloudDisk)
{
    if(m_CloudhttpClient)
    {
        QString url = m_apiUrl + "/resource/list";
        QJsonObject obj;
        ...
        AESCryptManager aESCryptManager(YMUserBaseInformation::appKey.toStdString());// AES加密
        QString msgen = QString::fromStdString(aESCryptManager.EncryptionAES(msg.toStdString()));
        qDebug() << "=======url=" << url <<",msg=" << msg << ",msgen=" << msgen;
        QJsonObject paramObj;
        paramObj.insert("appid", YMUserBaseInformation::appId);
        ...
        QJsonObject jsonObj = m_CloudhttpClient->syncRequestMsg(url, parammsg, YMUserBaseInformation::miniClassToken, 5000);
        if(!jsonObj.empty())
        {
    ...
    {
        qWarning()<< "getCloudDiskList failed, m_httpClint is null";
        return -1;
    }
    return 0;
}

int YMLessonManagerAdapter::upLoadCourseware(QString upFileMark, QString roomId, QString userId, QString fileUrl, long fileSize, QString apiUrl, QString appId)
{
    if(m_CloudhttpClient)
    {
        ...
        QString url = m_apiUrl + "/resource/app/save";
        QJsonObject jsonObj = m_CloudhttpClient->syncRequestMsg(url, parammsg, YMUserBaseInformation::miniClassToken, 5000);
...
int YMLessonManagerAdapter::findFileStatus(QString coursewareId, QString apiUrl, QString appId)
{
    if(m_CloudhttpClient)
    {
        QJsonObject obj;
        ...
        AESCryptManager aESCryptManager(YMUserBaseInformation::appKey.toStdString());
        QString msgen = QString::fromStdString(aESCryptManager.EncryptionAES(msg.toStdString()));
        ...
        QString url = m_apiUrl + "/resource/find/status";
        QJsonObject jsonObj = m_CloudhttpClient->syncRequestMsg(url, parammsg, YMUserBaseInformation::miniClassToken, 5000);
        qDebug() << "======result=" << jsonObj;
        if(!jsonObj.empty())
        {
...
int YMLessonManagerAdapter::deleteCourseware(QString coursewareId, QString roomId, QString apiUrl, QString appId)
{
    if(m_CloudhttpClient)
    {
        QJsonObject obj;
        ...
        QString url = m_apiUrl + "/resource/cancel";
        QJsonObject jsonObj = m_CloudhttpClient->syncRequestMsg(url, parammsg, YMUserBaseInformation::miniClassToken, 5000);
        qDebug() << "======result=" << jsonObj;
        if(!jsonObj.empty())
        {
...
long  YMLessonManagerAdapter::getFileSize(const QString &filePath)
{
    std::string filepath = std::string((const char*)filePath.toLocal8Bit());
