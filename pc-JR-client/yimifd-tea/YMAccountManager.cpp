...
void YMAccountManager::getLatestVersion()
{
    ...
    if(!YMUserBaseInformation::m_bIsPublicTest)
    {
        ...
        reqParam.insert("appName", "tea_pc_1v1"); //生产版本
    }
    else
    {
...
{
    return m_firmwareData.value("downloadUrl").toString();
}
...
void YMAccountManager::getMiniClassToken()
{
    ...
    QString postData = QString("{\"appid\": \"3edaf2877c994a4e8739dbe34411e7d7\",\"certificate\": \"%2\",\"userId\": \"%1\",\"userType\": 2}").arg(YMUserBaseInformation::id).arg(aesCert);//
