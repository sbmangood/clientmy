...
MouseArea{
    ...
    onIsStratClassChanged:
    {
        if(isStratClass)
        {
            if(isPlaying)
            {
                ...
                sigPlayerMedia("audio","play",parseInt(seekTime / 1000),filePath)
            }
        }
    }
    ...
    MouseArea {
        ...
        onClicked:{//更换播放图标
            ...
            if(isPlaying) {
                mediaPlayer.pause();
                sigPlayerMedia("audio","pause",currentTime,filePath);
            }
            else {
                mediaPlayer.play();
                sigPlayerMedia("audio","play",currentTime,filePath);
            }
            isPlaying = !isPlaying;
        }
    ...
    Video {//MediaPlayer
        ...
        onPlaybackStateChanged: {
            ...
            case MediaPlayer.StoppedState:
                sigPlayerMedia("audio","stop","0",filePath);
    ...
    function ymAudioPlayerManagerPlayFileByUrl(fileUrl,fileName,startTime,audioPath) {
    ...
    function setPlayerAudioBuffer(fileName){
        ...
        for(var i = 0; i < audioPlayerBuffer.length;i++){
            if(audioPlayerBuffer[i].fileName == fileName){
                ...
                sigPlayerMedia("audio","play",playerTime,filePath);
