...
void SocketManager::initSocketMessageCtrl(const QVariantList &goodIpList)
{
    ...
    int port, udpPort;
    bool isTcpProtocol;
    m_socketMessageCallBack->getPersonData(address, udpPort, port, isTcpProtocol);

    //初始化mar
    m_tcpSocket = new TcpSocket(m_goodIpList, address, port);
...
{
    qDebug() << " asynSendMessage -----"<< message;
...
void SocketManager::justChangeIp(bool isSuccess)
{
    ...
    socketJsonObj.insert("currentSocketIp",address);
    socketJsonObj.insert("errMsg",QStringLiteral("连接失败"));
    socketJsonObj.insert("socketIp",address);
//    YMQosManager::gestance()->addBePushedMsg("XBKsocketDisconnect", socketJsonObj);
    ...
    //判断是否切换到 http 协议
    int serverDelay, serverLost;
    m_socketMessageCallBack->getServerQosInfo(serverDelay, serverLost);
    if(( serverLost >= 5 || serverDelay > 50  ||  m_currentReloginTimes == 2 ) /*&& TemporaryParameter::gestance()->m_isStartClass == true*/)
    {
        m_currentReloginTimes = 0;
...
void SocketManager::autoChangeIp()
{
    ...
    if( m_goodIpList.size() > 0 )
    {
        ...
        for(int a = 0; a < m_goodIpList.size(); a++ )
        {
            ...
            if(address == tempMap["ip"].toString())
            {
        ...
        {
            //没有被连接过
            address = m_goodIpList.at(0).toMap()["ip"].toString();
            port = m_goodIpList.at(0).toMap()["port"].toInt();
            udpPort = m_goodIpList.at(0).toMap()["udpPort"].toInt();
        }
        else
        {
            //被连接过位置的下一个 或第一个
            ...
            {
                address = m_goodIpList.at(tempHasConnect + 1).toMap()["ip"].toString();
                port = m_goodIpList.at(tempHasConnect + 1).toMap()["port"].toInt();
                udpPort = m_goodIpList.at(tempHasConnect + 1).toMap()["udpPort"].toInt();
            }
            else
            {
                address = m_goodIpList.at(0).toMap()["ip"].toString();
        ...
        if(m_currentAutoChangeIpTimes == m_goodIpList.size() )
        {
            isTcpProtocol = !isTcpProtocol;;
        }
        m_socketMessageCallBack->onPersonData(address, udpPort, port, isTcpProtocol);
...
void SocketManager::restGoodIpList()
{
    ...
    if(tempHasConnect == -1)
    {
        //不包含 默认ip
        QVariantMap tempMap ;
        tempMap.insert("port", QString::number(port));
        tempMap.insert("udpPort", QString::number(udpPort));
        tempMap.insert("ip", address);
