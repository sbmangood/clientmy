                                       ...
                                       QString apiVersion, QString appVersion, QString token, QString strDllFile, QString strAppName, QString &logFilePath)
{
    ...
    qDebug()<<"====getCurrentEnvironmentType()==" <<YMHttpClientUtils::getInstance()->getCurrentEnvironmentType();
    int iResult = 0;
    ...
    //小组课设为直播模式 CHANNEL_PROFILE_LIVE_BROADCASTING，其他设置为通信模式 CHANNEL_PROFILE_COMMUNICATION
    ...
    {
        if(m_role == TEACHER || STUDENT)
        {
            m_rtcEngine->setClientRole(CLIENT_ROLE_BROADCASTER, NULL);
        }
        else
        {
            m_rtcEngine->setClientRole(CLIENT_ROLE_AUDIENCE, NULL);
        }
    }
    ...
    if(m_courseType == SMALL_GROUP)
    {
        m_rtcEngine->setVideoProfile(agora::rtc::VIDEO_PROFILE_120P, false);
    }
    else
    {
        m_rtcEngine->setVideoProfile(agora::rtc::VIDEO_PROFILE_180P, false);
    }
