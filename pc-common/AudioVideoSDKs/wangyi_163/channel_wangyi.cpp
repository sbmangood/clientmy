...
// 得到视频帧数据
void chanel_wangyi::AddVideoFrame(bool capture, unsigned __int64  time, const char* data, unsigned int size, unsigned int width, unsigned int height, const std::string& json, FrameType frame_type)
{
    ...
    timeb time_now;
    ftime(&time_now);
    unsigned __int64  cur_timestamp = time_now.time * 1000 + time_now.millitm;
    ...
    else
    {
        ...
        if (it != recv_video_pic_list_.end() && it->second )
        {
