#include "AudioVideoManager.h"

AudioVideoManager::AudioVideoManager(QObject *parent): QObject(parent), m_processingchannel(NULL), m_isSuccess(false)
{
...
// 切换频道
void AudioVideoManager::changeChanncel(CHANNEL_TYPE currentChannel, QString videoType, QString microphoneState, QString cameraState,
...
// 设置留在教室
void AudioVideoManager::setStayInclassroom()
{
    exitChannel();
}

// 退出频道
void AudioVideoManager::exitChannel()
{
...
QString AudioVideoManager::getDefaultDevicesId(QString deviceKey)
{
    ...
    qDebug() << "AudioVideoManager::getDefaultDevicesId" << deviceSetting.value(deviceKey).toString() << __LINE__;
    return deviceSetting.value(deviceKey).toString();
}

// 关闭本地音频
void AudioVideoManager::closeLocalAudio()
{
    if(NULL != m_processingchannel)
    {
        m_processingchannel->closeLocalAudio();
...
// 打开本地音频
void AudioVideoManager::openLocalAudio()
{
    if(NULL != m_processingchannel)
    {
        m_processingchannel->openLocalAudio();
...
// 关闭本地视频
void AudioVideoManager::closeLocalVideo()
{
    if(NULL != m_processingchannel)
    {
        m_processingchannel->closeLocalVideo();
...
// 打开本地视频
void AudioVideoManager::openLocalVideo()
{
    if(NULL != m_processingchannel)
    {
        m_processingchannel->openLocalVideo();
...
void AudioVideoManager::slotCreateRoomFail()
{
    m_isSuccess = false;
    emit sigAisleFinish(m_isSuccess);
}

void AudioVideoManager::slotCreateRoomSuccess()
{
    m_isSuccess = true;
    emit sigAisleFinish(m_isSuccess);
}
