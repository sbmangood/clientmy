...
PingbackManager::PingbackManager(QObject *parent) :
    QObject(parent),
    company_(""),
    business_(""),
    appversion_(""),
...
void PingbackManager::SendEvent(const QString &eventId, YimiLogType logtype, const QMap<QString,QString> &extrainfos ){
       ...
       }else if (company_ == "JRONLINE"){
            baseinfo->set_company(LogEntry_Company_JRONLINE);
       }
       ...
       } else if(logtype == LOGTYPE_DEFAULT){
           baseinfo->set_type(LogEntry_LogType_LOGTYPE_DEFAULT);
       }
       ...
       if(net_type_ =="G2"){
        ...
        baseinfo->set_operatortype(LogEntry_OperatorType_OPERATOR_UNKNOW);
            ...
            auto live  = _logentry.mutable_liveinfo();
             if(!liveinfo_.isEmpty()){
                ...
                if(liveinfo_.contains("lessonType")){
                 ...
                 if(liveinfo_.contains("classType")){
                     QString  classtype = liveinfo_.value("classType").toString();
                     live->set_classtype(classtype.toStdString());
...
QString PingbackManager::GetPushUrl(){
       QString  pushurl = "https://galaxy-burypoint-pb.oneonline.com.cn/buryPoint/pc";
       if(!QFile::exists("Qtyer.dll")){
              pushurl = "https://galaxy-bp-prod.oneonline.com.cn/buryPoint/pc";
       }
         ...
         if(environmentType == 1){
               ...
               pushurl = "https://galaxy-burypoint-pb.oneonline.com.cn/buryPoint/pc";
          }
